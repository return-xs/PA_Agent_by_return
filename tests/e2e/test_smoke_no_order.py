"""E2E smoke test ‚Ä?no-order path: stage2 returns ‰∏ç‰∏ãÂç?
...
def test_no_order_shows_no_trade_conclusion(qtbot, tmp_path):
    """When stage2 returns ‰∏ç‰∏ãÂç? DecisionPanel shows that conclusion."""
    ...
    # DecisionPanel should show ‰∏ç‰∏ãÂç?
    conclusion_text = window._decision_panel._conclusion_label.text()
    assert "‰∏ç‰∏ãÂç? in conclusion_text, (
        f"Expected ‰∏ç‰∏ãÂç?conclusion, got: {conclusion_text!r}"
    )
    ...
    assert len(infinite_lines) == 0, (
        f"Expected no InfiniteLine items for ‰∏ç‰∏ãÂç? found {len(infinite_lines)}"
    )
